...
int ContourToolProcess::initTask(){
	HI_S32 s32Ret = HI_SUCCESS;
	int i, j;
	if (!(pstTemplateMatch&&pcontour_parameter)){
		KOYO_LOG_ERROR("Error: pstTemplateMatch: %p, pcontour_parameter: %p.\n", pstTemplateMatch, pcontour_parameter);
		return;
	}

	KOYO_LOG_INFO("search angle: %d.\n", pcontour_parameter->angle_range);
	/* 配置轮廓查找参数 */
	memcpy(&(pstTemplateMatch->contour_parameter), pcontour_parameter, sizeof(Koyo_Tool_Contour_Parameter));

	/* 设置每层金字塔的搜索区域 */     //此部分只用到顶层的搜索区域，在加入搜索框，在4层搜索时需要用到，以加快搜索速度
	for (i = 0; i < MAX_NUM_PYRAMID; i++) {
		...
		region.x = pcontour_parameter->search_rect_x;
		region.y = pcontour_parameter->search_rect_y;
		region.width = pcontour_parameter->search_rect_width;
		region.height = pcontour_parameter->search_rect_height;

		pstTemplateMatch->rect_roi[i].x = region.x >> i;
		pstTemplateMatch->rect_roi[i].y = region.y >> i;
		pstTemplateMatch->rect_roi[i].width = region.width >> i;
		pstTemplateMatch->rect_roi[i].height = region.height >> i;
	}

}

int ContourToolProcess::doTask(IVSContourResult& ivsContourResult){
	Tool_Utility *tool_utility = palg_handle->tool_utility;
	...
	/* 运行前保证 palg_handle->run_tool_param*/
	struct timeval tt[2];
	struct timeval tt1[2];
	struct timeval tt2[2];
	struct timeval tt3[2];
	if (palg_handle == NULL){
		KOYO_LOG_ERROR("Error: null ptr: %p", palg_handle);
		return;
	}
	start(tt);

	// 需要重新计算局部的搜索范围
	Koyo_Template_Match_Info *pstTemplateMatch = (Koyo_Template_Match_Info *)palg_handle->run_tool_param;
	if (!(palg_handle->contour_param->search_rect_x == 0 && palg_handle->contour_param->search_rect_y == 0
		&& palg_handle->contour_param->search_rect_width == IMAGE_MAX_WIDTH
		&& palg_handle->contour_param->search_rect_height == IMAGE_MAX_HEIGHT)){
		for (int i = 0; i < MAX_NUM_PYRAMID; i++) {

			Rect region;

			region.x = palg_handle->contour_param->search_rect_x;
			region.y = palg_handle->contour_param->search_rect_y;
			region.width = palg_handle->contour_param->search_rect_width;
			region.height = palg_handle->contour_param->search_rect_height;
			...
			KOYO_LOG_INFO("region.x:%d\n", pstTemplateMatch->rect_roi[i].x);
			KOYO_LOG_INFO("region.y:%d\n", pstTemplateMatch->rect_roi[i].y);
			KOYO_LOG_INFO("region.width:%d\n", pstTemplateMatch->rect_roi[i].width);
			KOYO_LOG_INFO("region.height:%d\n", pstTemplateMatch->rect_roi[i].height);
		}

	}

	// 对于不同的灵敏度来重新计算每个工具的canny图像
	int lowThreshold = palg_handle->contour_param->sensitivity_Low_Threshold;
	...
	start(tt1);
	for (INT32 l = 0; l < MAX_NUM_PYRAMID; ++l) {

		KOYO_LOG_INFO("\n\nthe low threshold is %d, the high threshold is %d\n\n", lowThreshold, highThreshold);
		get_contour_binary_cannyback(&(tool_utility->search_region[l]),
			&(tool_utility->search_region_binary[l]), 0,
			lowThreshold, highThreshold); //todo:the thereshold need to adjus
		s32Ret = HI_MPI_SYS_MmzFlushCache(tool_utility->search_region_binary[l].u32PhyAddr[0],
			tool_utility->search_region_binary[l].pu8VirAddr[0],
			tool_utility->search_region_binary[l].u16Stride[0] *
			tool_utility->search_region_binary[l].u16Height);
		if (s32Ret != HI_SUCCESS) {
			KOYO_LOG_ERROR("HI_MPI_SYS_MmzFlushCache fail,Error(%#x)\n", s32Ret);
			return;
		}
		hi_ive_dilate(&(tool_utility->search_region_binary[l]), &(tool_utility->dilated_search_region_binary[l]));

		hi_ive_integ_at(&(tool_utility->search_region_binary[l]), &(tool_utility->image_search_integ[l]), 0, 0, &integ_ctrl_s);

		s32Ret = HI_MPI_SYS_MmzFlushCache(tool_utility->dilated_search_region_binary[l].u32PhyAddr[0],
			tool_utility->dilated_search_region_binary[l].pu8VirAddr[0],
			tool_utility->dilated_search_region_binary[l].u16Stride[0] *
			tool_utility->dilated_search_region_binary[l].u16Height);
		if (s32Ret != HI_SUCCESS) {
			KOYO_LOG_ERROR("HI_MPI_SYS_MmzFlushCache fail,Error(%#x)\n", s32Ret);
			return;
		}
		s32Ret = HI_MPI_SYS_MmzFlushCache(tool_utility->image_search_integ[l].u32PhyAddr[0],
			tool_utility->image_search_integ[l].pu8VirAddr[0],
			tool_utility->image_search_integ[l].u16Stride[0] *
			tool_utility->image_search_integ[l].u16Height);
		if (s32Ret != HI_SUCCESS) {
			KOYO_LOG_ERROR("HI_MPI_SYS_MmzFlushCache fail,Error(%#x)\n", s32Ret);
			return;
		}
	...
	s32Ret = HI_MPI_SYS_MmzFlushCache(tool_utility->search_region_binary[MAX_NUM_PYRAMID - 2].u32PhyAddr[0],
		tool_utility->search_region_binary[MAX_NUM_PYRAMID - 2].pu8VirAddr[0],
		tool_utility->search_region_binary[MAX_NUM_PYRAMID - 2].u16Stride[0] *
		tool_utility->search_region_binary[MAX_NUM_PYRAMID - 2].u16Height);
	if (s32Ret != HI_SUCCESS) {
		KOYO_LOG_ERROR("HI_MPI_SYS_MmzFlushCache fail,Error(%#x)\n", s32Ret);
		return;
	}
	s32Ret = HI_MPI_SYS_MmzFlushCache(tool_utility->search_region_binary[MAX_NUM_PYRAMID - 1].u32PhyAddr[0],
		tool_utility->search_region_binary[MAX_NUM_PYRAMID - 1].pu8VirAddr[0],
		tool_utility->search_region_binary[MAX_NUM_PYRAMID - 1].u16Stride[0] *
		tool_utility->search_region_binary[MAX_NUM_PYRAMID - 1].u16Height);
	if (s32Ret != HI_SUCCESS) {
		KOYO_LOG_ERROR("HI_MPI_SYS_MmzFlushCache fail,Error(%#x)\n", s32Ret);
		return;
	}
	...
	//    do_template_match(palg_handle->orignal_pic, (Koyo_Template_Match_Info *)palg_handle->run_tool_param, palg_handle);
	start(tt3);

	bool is_global = false;
	bool is_highspeed = false;
	...
	if (palg_handle->contour_param->algo_strategy == 0) {
		// 高精度
		is_highspeed = false;
	}
	else {
...
int do_create_template(Koyo_Contour_Template_Runtime_Param *koyo_contour_template_runtime_param, char *buf)
{
	...
	KOYO_LOG_INFO("npyramid:%d\n", koyo_contour_template_runtime_param->run_time_npyramid);
	index += sizeof(koyo_contour_template_runtime_param->run_time_npyramid);

	if ((koyo_contour_template_runtime_param->run_time_npyramid > MAX_NUM_PYRAMID)) {
		KOYO_LOG_INFO("ERROR: wrong template data, run_time_npyramid: %d is too high, check buf corrected.\n",
			koyo_contour_template_runtime_param->run_time_npyramid);
		return -1;
	}

	// 分配内存空间
	koyo_contour_template_runtime_param->search_angel_nstep = (float*)(buf + index);
	index += sizeof(float)* koyo_contour_template_runtime_param->run_time_npyramid;

	koyo_contour_template_runtime_param->search_rect_width = (UINT16*)(buf + index);
	for (int k = 0; k < koyo_contour_template_runtime_param->run_time_npyramid; ++k) {
		koyo_contour_template_runtime_param->search_rect_width[k] *= 2;
	}
	index += sizeof(UINT16)* koyo_contour_template_runtime_param->run_time_npyramid;

	koyo_contour_template_runtime_param->tpls = (TemplateStruct **)malloc(sizeof(TemplateStruct *)* koyo_contour_template_runtime_param->run_time_npyramid); // 分配指针数组
	//koyo_contour_template_runtime_param->tpls = (TemplateStruct **)(buf + index);
	//index += sizeof(TemplateStruct*) * koyo_contour_template_runtime_param->run_time_npyramid;

	// 拷贝模板数据
	for (i = 0; i < koyo_contour_template_runtime_param->run_time_npyramid; ++i) {

		//std::vector<TemplateStruct> tpl_arr;

		UINT16 tpl_size = *((UINT16*)&buf[index]);
		KOYO_LOG_INFO("tpl size:%d.\n", tpl_size);
		index += sizeof(UINT16);

		TemplateStruct *tpl_arr = (TemplateStruct *)malloc(sizeof(TemplateStruct)* tpl_size);

		if (!tpl_arr) {
			KOYO_LOG_ERROR("Error malloc.\n");
		}

		KOYO_LOG_INFO("tpl_size: %d\n", tpl_size);
		for (j = 0; j < tpl_size; ++j) {
			TemplateStruct tpl;
			...
			// 拷贝特征数据
			KOYO_LOG_TRACE("tpl no ofcoordinate: %d\n", tpl.noOfCordinates);
			tpl.cordinates = (IPoint *)(buf + index);
			index += sizeof(IPoint)* tpl.noOfCordinates;
			KOYO_LOG_TRACE("tpl cordinate[%d]:%d.\n", j, tpl.noOfCordinates);

			tpl.edgeDerivativeX = (float*)(buf + index);
			index += (sizeof(float)* tpl.noOfCordinates);
			tpl.edgeDerivativeY = (float*)(buf + index);
			index += (sizeof(float)* tpl.noOfCordinates);

			//	tpl_arr.push_back(tpl);
			tpl_arr[j] = tpl;
		}
		KOYO_LOG_TRACE("tpl cordinate:%d.\n", tpl_arr[0].noOfCordinates);
		koyo_contour_template_runtime_param->tpls[i] = tpl_arr;
...
{
	/* 调用前保证palg_handle->run_tool_param已经被正确初始化了 */
	if (!palg_handle){
		KOYO_LOG_ERROR("Error: palg_handle: %p.\n", palg_handle);
	}
	if (!buf) {
		KOYO_LOG_ERROR("Error: NULL Pointer.\n");
	}

#if 0
	char filename[100];
	sprintf(filename, "../image/origin_pic_%s", __FUNCTION__);
	board_tools_write_to_file_u8c1_formated(filename, palg_handle->orignal_pic->u16Stride[0], palg_handle->orignal_pic->u16Height, palg_handle->orignal_pic->pu8VirAddr[0]);
#endif
	KOYO_LOG_INFO("first coordinate: %d\n", *(int*)(buf + 1 + 4 * 4 + 2 * 4 + 2 + 1));
	Koyo_Contour_Template_Runtime_Param koyo_contour_template_runtime_param;
	do_create_template(&koyo_contour_template_runtime_param, buf);
	//do_create_template(palg_handle->orignal_pic, (Koyo_Template_Match_Info *)palg_handle->run_tool_param);
	// 模板信息都在 (Koyo_Template_Match_Info *)palg_handle->run_tool_param 内
	Koyo_Template_Match_Info * run_tool_param = (Koyo_Template_Match_Info *)palg_handle->run_tool_param;

	run_tool_param->run_time_npyramid = koyo_contour_template_runtime_param.run_time_npyramid;
	run_tool_param->search_angel_nstep = koyo_contour_template_runtime_param.search_angel_nstep;
	run_tool_param->tpls = koyo_contour_template_runtime_param.tpls;
	run_tool_param->search_rect_width = koyo_contour_template_runtime_param.search_rect_width;

	KOYO_LOG_INFO("create template done!.\n");
	/* 建立模板不需要返回信息 */
}
...
INT8 match_template_one_level(IVE_IMAGE_S *search_region_bin, IVE_IMAGE_S *dilated_search_region_bin, Koyo_Template_Match_Info *template_match_info,
	...
	if (level == template_match_info->run_time_npyramid - 1){ // 顶层
...
#if 0
		IVE_IMAGE_S tmp_test;
		SAMPLE_COMM_IVE_CreateImage(&tmp_test, IVE_IMAGE_TYPE_U8C1, search_region_bin->u16Width, search_region_bin->u16Height);
		memset(tmp_test.pu8VirAddr[0], 0, tmp_test.u16Stride[0] * tmp_test.u16Height);
		//原始图片
		for (int l = 0; l < tmp_test.u16Height; ++l) {
			for (int m = 0; m < tmp_test.u16Width; ++m) {
				tmp_test.pu8VirAddr[0][l*tmp_test.u16Stride[0] + m] = search_region_bin->pu8VirAddr[0][l*tmp_test.u16Stride[0] + m];
			}
		}
		//        maxang = 42;
		//        maxi = 42;
		//        maxj = 48;
		//搜索框
		//        for (int l = maxi-search_rect_width_half; l < maxi + search_rect_width_half; ++l) {
		//            for (int m = maxj-search_rect_width_half; m < maxj + search_rect_width_half; ++m) {
		//                if(l == maxi-search_rect_width_half || m == maxj-search_rect_width_half ||
		//                        l == maxi+search_rect_width_half || m == maxj + search_rect_width_half
		//                   /*|| search_region_bin->pu8VirAddr[0][search_region_bin->u16Stride[0]*l + m]*/)
		//                    tmp_test.pu8VirAddr[0][l*tmp_test.u16Stride[0]+m] = 2;
		//            }
		//        }
		//        board_tools_write_to_file_u8c1_formated("../image/congour_edge_recovery_ori",tmp_test.u16Stride[0],tmp_test.u16Height,tmp_test.pu8VirAddr[0]);
		//        memset(tmp_test.pu8VirAddr[0],0,tmp_test.u16Stride[0]*tmp_test.u16Width);
		//画模板
		for (int n = 0; n < tpl_one_level_p[maxang].noOfCordinates; ++n) {
			int cordy = maxi + tpl_one_level_p[maxang].cordinates[n].y;
			int cordx = maxj + tpl_one_level_p[maxang].cordinates[n].x;
			if (cordy >= 0 && cordy < tmp_test.u16Height&&cordx >= 0 && cordx < tmp_test.u16Width)
				tmp_test.pu8VirAddr[0][tmp_test.u16Stride[0] * cordy + cordx] = 3;
			else{
				//                KOYO_LOG_INFO("contour coord exceed border. ignore.\n");
			}
		}
		board_tools_write_to_file_u8c1_formated("../image/contour_edge_recovery", tmp_test.u16Stride[0], tmp_test.u16Height, tmp_test.pu8VirAddr[0]);
#endif
		//        exit(0);

		//        for (int i = 0; i < MAX_HEAP_SIZE; ++i) {
		//            KOYO_LOG_INFO("%d, pos: %d,%d, score: %f\n",
		//                       i+1,
		//                       candidate_heap_out->data[i]->position.y,
		//                       candidate_heap_out->data[i]->position.x,
		//                       candidate_heap_out->data[i]->score);
		//            if(i * 2 + 1 < MAX_HEAP_SIZE){
		//                if(candidate_heap_out->data[i]->score > candidate_heap_out->data[i*2+1]->score){
		//                    KOYO_LOG_INFO("error heap.\n");
		//                }
		//            }
		//            if((i + 1)* 2 < MAX_HEAP_SIZE){
		//                if(candidate_heap_out->data[i]->score > candidate_heap_out->data[(i + 1)* 2]->score){
		//                    KOYO_LOG_INFO("error heap.\n");
		//                }
		//            }
		//        }
		...
		//            hi_ive_integ_at(search_region_bin, &integ, 0, 0, &integ_ctrl_s);
		//            Rect match_rect;
		//            match_rect.height = search_region_bin->u16Height-1;
		//            match_rect.width = search_region_bin->u16Width-1;
		//            match_rect.x = 0;
		//            match_rect.y = 0;
		//            int points_level = get_contour_npoints_by_rect(&integ, &match_rect);
		//            KOYO_LOG_INFO("******************INFO*******************points_level %d: %d\n", level, points_level);
		...
		if (level == com_level){
...
#if 0
			maxang = candidate_heap_out->data[0]->angel_idx;
			maxi = candidate_heap_out->data[0]->position.y;
			maxj = candidate_heap_out->data[0]->position.x;
			IVE_IMAGE_S tmp_test;
			SAMPLE_COMM_IVE_CreateImage(&tmp_test, IVE_IMAGE_TYPE_U8C1, search_region_bin->u16Width, search_region_bin->u16Height);
			...
			board_tools_write_to_file_u8c1_formated("../image/contour_edge_recovery_ori", tmp_test.u16Stride[0], tmp_test.u16Height, tmp_test.pu8VirAddr[0]);
#endif
			////            exit(0);
		}
		stop(tt);
